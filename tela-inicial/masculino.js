...
function pesquisarProdutos() {
    ...
    fetch("http://localhost:3006/api/get/iniciais")
        ...
        .then(result => {
            if (result.success) {
                const masculinoContainer = document.querySelector(".produtos-container.masculino");
                masculinoContainer.innerHTML = ''; // Limpa os produtos atuais antes de exibir os filtrados

                const produtosFiltrados = result.data.filter(produto =>
                    produto.nome.toLowerCase().includes(termoPesquisa) && produto.categoria === 'masculino'
                );
